...
use cedar_policy::ffi::{
    ...
    PolicySet as PolicySetFFI, Schema as FFISchema,
...
pub extern "C" fn cedar_version() -> *const u32 {
    return_string(&cedar_policy::ffi::get_sdk_version())
}
...
fn host_bridge(name: &str, args_json: &str) -> Result<String, String> {
    let name_bytes = name.as_bytes();
    let args_bytes = args_json.as_bytes();

    unsafe {
        let name_ptr = alloc(name_bytes.len());
        let args_ptr = alloc(args_bytes.len());
        core::ptr::copy_nonoverlapping(name_bytes.as_ptr(), name_ptr, name_bytes.len());
        core::ptr::copy_nonoverlapping(args_bytes.as_ptr(), args_ptr, args_bytes.len());

        let result_wide = host_extension_call(
            name_ptr as i32, name_bytes.len() as i32,
            args_ptr as i32, args_bytes.len() as i32,
        );

        dealloc(name_ptr, name_bytes.len());
        dealloc(args_ptr, args_bytes.len());

        let result_ptr = (result_wide & 0xFFFFFFFF) as usize as *const u8;
        let result_len = ((result_wide >> 32) & 0xFFFFFFFF) as usize;
        let result_str = core::str::from_utf8_unchecked(
            core::slice::from_raw_parts(result_ptr, result_len),
        );
        let result = result_str.to_string();
        dealloc(result_ptr as *mut u8, result_len);
        Ok(result)
    }
}

/// Called once at startup to wire the host callback into the Cedar extension.
#[no_mangle]
pub extern "C" fn cedar_init_host_extensions() -> *const u32 {
    cedar_policy_core::extensions::host::set_host_callback(host_bridge);
    return_string("OK")
}
